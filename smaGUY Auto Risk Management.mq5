...
NetGroup CalculateNetGroup()
  {
   ...
   if(group.net_volume > 0.001)
      group.direction = "BUY";
   else
      if(group.net_volume < -0.001)
         group.direction = "SELL";
      else
         group.direction = "HEDG";
   return group;
  }
...
int CalculateFeasiblePartials(double volume)
  {
   double min_lot = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
   double lot_step = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_STEP);
   if(min_lot <= 0 || lot_step <= 0 || volume < min_lot)
      return 0;
...
// Total steps = feasible partials + 1 (for final TP)
   ...
   if(ArraySize(step_plan) != total_steps)
      return avg_tp;
...
// Include manual partial positions at their TP
   for(int i = 0; i < ArraySize(manual_partials); i++)
     {
      if(manual_partials[i].current_volume <= 0)
         continue;
   ...
   if(steps_completed < 0)
      steps_completed = 0;
   if(steps_completed > feasible_partials)
      steps_completed = feasible_partials;
...
double CalculateTotalReward(NetGroup &group, int feasible_partials)
  {
   ...
   if(weighted_tp <= 0 || group.original_volume <= 0)
      return 0;
   double avg_entry = group.weighted_entry / group.total_volume;
   bool isBuy = (group.direction == "BUY" || (group.direction == "HEDG" && group.net_volume >= 0));
...
double CalculateTotalRisk(NetGroup &group)
  {
   if(group.weighted_sl <= 0 || group.total_volume <= 0)
      return 0;
   double avg_entry = group.weighted_entry / group.total_volume;
...
void ManagePartials()
  {
   ...
   double lot_step = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_STEP);
   double min_lot = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
   if(point <= 0 || lot_step <= 0 || min_lot <= 0)
      return;
   double avg_entry = group.weighted_entry / group.total_volume;
   double avg_tp = (group.weighted_tp > 0) ? (group.weighted_tp / group.total_volume) : 0;
   if(avg_tp <= 0)
...
// Analyze manual partials
   ...
   double auto_partial_volume = 0;
   AnalyzeManualPartials(group, manual_partials, auto_partial_volume);
   if(auto_partial_volume < min_lot)
      return;
   ...
   for(int i = 0; i < group.ticket_count && remaining_to_close >= min_lot; i++)
     {
      ...
      if(is_manual_partial)
         continue; // Skip manual partial positions
      double pos_vol = PositionGetDouble(POSITION_VOLUME);
      if(pos_vol < min_lot)
         continue;
      ...
      for(int i = 0; i < group.ticket_count; i++)
        {
         ...
         if(!PositionSelectByTicket(ticket))
            continue;
         int idx = FindStateIndex(ticket);
         if(idx >= 0 && position_states[idx].be_applied)
            continue;
         ...
         if(should_modify)
           {
            if(trade.PositionModify(ticket, be_price, PositionGetDouble(POSITION_TP)))
              {
               if(idx >= 0)
                  position_states[idx].be_applied = true;
...
void InitializeDashboard()
  {
   ...
   if(InpSingleLineMode)
     {
      CreateLabel("Line1_Pos", 0);
      CreateLabel("Line1_PnL", 0);
      CreateLabel("Line1_SL", 0);
      CreateLabel("Line1_TP", 0);
     }
   else
     {
      CreateLabel("Line1", 0);
...
void UpdateDashboard()
  {
   ...
   double avg_tp = (group.weighted_tp > 0 && group.total_volume > 0) ? (group.weighted_tp / group.total_volume) : 0;
// === Sync TP/SL to raw weighted values (no partial logic) ===
   int digits = (int)SymbolInfoInteger(_Symbol, SYMBOL_DIGITS);
   double point = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
   for(int i = 0; i < group.ticket_count; i++)
     {
      ...
      double current_tp = PositionGetDouble(POSITION_TP);
      double current_sl = PositionGetDouble(POSITION_SL);
      bool tp_diff = (avg_tp > 0 && MathAbs(current_tp - avg_tp) > point);
      bool sl_diff = (avg_sl > 0 && MathAbs(current_sl - avg_sl) > point);
      if(tp_diff || sl_diff)
        {
         double new_tp = tp_diff ? NormalizeDouble(avg_tp, digits) : current_tp;
         double new_sl = sl_diff ? NormalizeDouble(avg_sl, digits) : current_sl;
         trade.PositionModify(ticket, new_sl, new_tp);
        }
     }
   ...
   if(total_risk > 0.001)
     {
      ...
      if(total_reward > 0.001)
         targetRR = DoubleToString(total_reward / total_risk, 1);
     }
         ...
         for(int i = 0; i < ArraySize(manual_partials); i++)
           {
            ...
            double reward = 0;
            if(isBuy)
               reward = (manual_partials[i].tp_price - avg_entry) / point * SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_VALUE) * manual_partials[i].current_volume;
            else
               reward = (avg_entry - manual_partials[i].tp_price) / point * SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_VALUE) * manual_partials[i].current_volume;
         ...
         if(feasible_partials > 0 && auto_partial_volume >= SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN))
           {
            ...
            for(int step = steps_completed; step < feasible_partials && total_steps > 0; step++)
              {
               ...
               double reward = 0;
               if(isBuy)
                  reward = (level_price - avg_entry) / point * SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_VALUE) * step_volume;
               else
                  reward = (avg_entry - level_price) / point * SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_VALUE) * step_volume;
               string line_name = "TM_Line_TP" + IntegerToString(total_tp_count);
               string label = "TP" + IntegerToString(total_tp_count) + " " +
            ...
            if(isBuy)
               final_reward = (avg_tp - avg_entry) / point * SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_VALUE) * final_volume;
            else
               final_reward = (avg_entry - avg_tp) / point * SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_VALUE) * final_volume;
            string line_name = "TM_Line_TP" + IntegerToString(total_tp_count);
            string label = "TP" + IntegerToString(total_tp_count) + " " +
...
void OnTick()
  {
   if(isOnTickRunning || !InpEnablePartials)
      return;
   isOnTickRunning = true;
   ManagePartials();
   isOnTickRunning = false;
  }
